...
\section{Predicting Donor's Choose}\label{predicting-donors-choose}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  What is the primary metric you care about in this task? Be sure to
  clearly state the question about why this is the case.
\item
  Is the column
  \texttt{teacher\_number\_of\_previously\_posted\_projects} a good
  predictor for the approval of the project? Use both a
  \texttt{KNearestNeighbors} and \texttt{LogisticRegression} model.
  Which model performs better? Can you select the best parameters for
  each? The best penalty for \texttt{LogisticRegression}? Form a
  pipeline that includes a scaling transformation? Compare this to a
  \texttt{DummyClassifier}?
\item
  What are the top 8 states in terms of raw number approved? The lowest?
  Show me with a nice barplot.
\item
  Are these states different from the number of proportion of
  applications approved by state? Show me.
\item
  Does your model improve with the inclusion of the
  \texttt{teacher\_prefix} column in a \texttt{LogisticRegression}
  model?
\item
  What is your best parameter for the training set with these inputs?
\item
  Construct a feature that is simply \texttt{STEM}, which is 1 if a
  scientific discipline is a part of the \texttt{subject\_subcategory}
  column, or 0 if not. Did your model improve?
\item
  What if you include the \texttt{project\_grade\_category} column? Is
  your model improved?
\item
  What if your client only cares about what's happening in New York. Is
  there a difference in the performance of a \texttt{LogisticRegression}
  model? \texttt{KNearestNeighbors}?
\end{enumerate}
